...
const createOrder = async (req, res, next) => {
  try {
    ...
    const {
      ...
      auth: {
        //Para enviar una autenticacion basica
        username: CLIENT,
        password: SECRET,
      },
    });
...
const createProduct = async (req, res, next) => {
  try {
    const product = {
      ...
      type: "DIGITAL",
      image_url:
        "https://www.impulsonegocios.com/wp-content/uploads/2019/04/donacion-organos.jpg",
...
const listProducts = async (req, res, next) => {
  try {
    ...
    const response = await axios.get(
      `${PAYPAL_API}/v1/catalogs/products?page_size=2&page=1&total_required=true`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${access_token}`,
...
const listPlans = async (req, res, next) => {
  try {
    ...
    console.log(access_token);
    const response = await axios.get(
      `${PAYPAL_API}/v1/billing/plans?product_id=PROD-1JR71498VV238292S&page_size=2&page=1&total_required=true`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${access_token}`,
...
const suscriptionDetail = async (req, res, next) => {
  ...
  try {
    ...
    console.log(access_token);
    const response = await axios.get(
      `${PAYPAL_API}/v1/billing/subscriptions/${id}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${access_token}`,
